...
namespace api.Repositories
{
    public class JogoRepository : IJogoRepository
    {
        ...
        public void Atualizar(Jogo jogo, List<int> GeneroIds, List<int> PlataformaIds)
        {
            ...
            var generosNovos = _context.Genero
                .Where(g => GeneroIds.Contains(g.GeneroID))
                .ToList();

            jogoBanco.Genero.Clear();
            foreach (var genero in generosNovos)
            {
                jogoBanco.Genero.Add(genero);
            }

            var plataformasNovas = _context.Plataforma
                .Where(p => PlataformaIds.Contains(p.PlataformaID))
                .ToList();

            jogoBanco.Plataforma.Clear();
            foreach (var plataforma in plataformasNovas)
            {
